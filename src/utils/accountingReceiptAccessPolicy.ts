...
/** 既存 Firestore に残る永続 URL フィールド名（新規保存しない） */
export const ACCOUNTING_RECEIPT_PERSISTENT_URL_FIELDS = [
  'downloadUrl',
  'imageUrl',
  'originalDownloadUrl',
  'ocrImageDownloadUrl',
...
export const stripPersistentAccountingReceiptUrls = <T extends Record<string, unknown>>(
  input: T,
): T => {
  const next = { ...input }
  for (const field of ACCOUNTING_RECEIPT_PERSISTENT_URL_FIELDS) {
    if (field in next) {
