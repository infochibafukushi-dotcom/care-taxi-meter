...
import { extractAreaName } from './address'
import { createJapanStartOfDayIsoFromDateParts } from './japanDate'
...
const createJapanStartOfDayIso = (dateValue: string) => {
  ...
  return createJapanStartOfDayIsoFromDateParts(year, month, day)
}

const createJapanNextDayStartIso = (dateValue: string) => {
  const { day, month, year } = parseDateValue(dateValue)
  return createJapanStartOfDayIsoFromDateParts(year, month, day + 1)
}
