          ...
          {activeAdminSection === "fare" ? (
            <div className="fare-accordion-stack">
              ...
              <details className="fare-settings-accordion">
                ...
                <div className="admin-settings-grid">
                  <fieldset>
                    <legend>時間制基本料金</legend>
                    <label>基本時間（分）<input min="1" type="number" value={settings.meterSettings.time.baseMinutes} onChange={(event) => updateTimeMeterFare("baseMinutes", event.target.value)} /></label>
                    <label>基本料金（円）<input min="0" type="number" value={settings.meterSettings.time.baseFareYen} onChange={(event) => updateTimeMeterFare("baseFareYen", event.target.value)} /></label>
                  </fieldset>
                  <fieldset>
                    <legend>時間制加算料金</legend>
                    <label>加算時間（分）<input min="1" type="number" value={Math.round(settings.meterSettings.time.additionalFare.unitSeconds / 60)} onChange={(event) => updateTimeMeterFare("additionalMinutes", event.target.value)} /></label>
                    <label>加算料金（円）<input min="0" type="number" value={settings.meterSettings.time.additionalFare.unitFareYen} onChange={(event) => updateTimeMeterFare("additionalFareYen", event.target.value)} /></label>
                  </fieldset>
                </div>
              </details>
              ...
              <details className="fare-settings-accordion">
                ...
                <div className="admin-settings-grid">
                  <fieldset className="admin-settings-wide">
                    ...
                    <button type="button" onClick={copyGpsFareToObd}>GPS料金をコピー</button>
                  </fieldset>
                  <fieldset>
                    <legend>OBD基本運賃設定</legend>
                    <label>初乗距離(km)<input min="0" step="0.001" type="number" value={settings.meterSettings.obd.basicFare.initialDistanceKm} onChange={(event) => updateObdBasicFare("initialDistanceKm", event.target.value)} /></label>
                    <label>初乗運賃(円)<input min="0" type="number" value={settings.meterSettings.obd.basicFare.initialFareYen} onChange={(event) => updateObdBasicFare("initialFareYen", event.target.value)} /></label>
                  </fieldset>
                  <fieldset>
                    <legend>OBD距離加算設定</legend>
                    <label>距離加算距離（m）<input min="1" step="1" type="number" value={Math.round(settings.meterSettings.obd.basicFare.additionalDistanceKm * 1000)} onChange={(event) => updateObdBasicFare("additionalDistanceKm", String(toPositiveNumber(event.target.value, 1) / 1000))} /></label>
                    <label>距離加算金額（円）<input min="0" type="number" value={settings.meterSettings.obd.basicFare.additionalFareYen} onChange={(event) => updateObdBasicFare("additionalFareYen", event.target.value)} /></label>
                  </fieldset>
                  <fieldset>
                    <legend>OBD時間加算設定</legend>
                    <label>低速判定速度（km/h）<input min="0" step="0.1" type="number" value={settings.meterSettings.obd.meterTimeFare.lowSpeedThresholdKmh} onChange={(event) => updateObdMeterTimeFare("lowSpeedThresholdKmh", event.target.value)} /></label>
                    <label>時間加算秒数（秒）<input min="1" step="1" type="number" value={settings.meterSettings.obd.meterTimeFare.unitSeconds} onChange={(event) => updateObdMeterTimeFare("unitSeconds", event.target.value)} /></label>
                    <label>時間加算金額（円）<input min="0" type="number" value={settings.meterSettings.obd.meterTimeFare.unitFareYen} onChange={(event) => updateObdMeterTimeFare("unitFareYen", event.target.value)} /></label>
                  </fieldset>
                </div>
              </details>
            </div>
